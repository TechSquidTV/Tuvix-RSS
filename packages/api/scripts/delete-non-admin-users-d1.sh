...
cd "$API_DIR"
USERS_JSON=$(wrangler d1 execute tuvix --config wrangler.toml.local --remote --command "SELECT id, name, email, role FROM user ORDER BY id;" --json --yes 2>&1 | grep -v "WARNING\|ratelimits\|Processing wrangler.toml\|update available\|Executing on" || true)
...
RESULT=$(wrangler d1 execute tuvix --config wrangler.toml.local --remote --command "$SQL_QUERY" --json --yes 2>&1 | grep -v "WARNING\|ratelimits\|Processing wrangler.toml\|update available\|Executing on" || true)
...
if echo "$RESULT" | grep -q '"success":\s*true'; then
  ...
  wrangler d1 execute tuvix --config wrangler.toml.local --remote --command "SELECT id, name, email, role FROM user ORDER BY id;" --json --yes 2>&1 | grep -v "WARNING\|ratelimits\|Processing wrangler.toml\|update available\|Executing on" | python3 -m json.tool 2>/dev/null || echo "$RESULT"
else
