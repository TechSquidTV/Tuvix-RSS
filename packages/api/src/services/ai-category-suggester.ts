            ...
            const { object } = await generateObject({
                ...
                schema: z.object({
                    existingMatches: z.array(
                        z.object({
                            categoryId: z.number().describe('The ID of the existing category'),
                            confidence: z.number().min(0).max(1).describe('Confidence score from 0 to 1'),
                        })
                    ).describe('Matching categories from the user\'s list'),
                    newSuggestions: z.array(
                        z.object({
                ...
                prompt: "Based on the provided context, suggest relevant categories for this RSS feed.",
                system: systemPrompt,
            });
